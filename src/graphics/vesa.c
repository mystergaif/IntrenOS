...
int vesa_init(uint32 width, uint32 height, uint32 bpp) {
    bios32_init();
    console_printf("initializing vesa vbe 2.0\n");
    if (!get_vbe_info()) {
        console_printf("No VESA VBE 2.0 detected\n");
        return -1;
    }
    // set this to 1 to print all available modes to console
    #define PRINT_MODES 0
    #if PRINT_MODES
        console_printf("Press UP and DOWN arrow keys to scroll\n");
        console_printf("Modes:\n");
        vbe_print_available_modes();
        return 1;
    #else
        g_selected_mode = vbe_find_mode(width, height, bpp);
        if (g_selected_mode == -1) {
            console_printf("failed to find mode for %d-%d\n", width, height);
            return -1;
        }
        console_printf("\nselected mode: %d \n", g_selected_mode);
